                     ...
                     which, ...)
{
  if(class(x) == "Smcmc")
  {
    ...
    {
      varnames <- paste("Density of" ,as.character(1:dim(x$chains[[1]])[2]))
    }else{
      varnames <- x$varnames
    }
    data <- x$stacked
    ...
    if(class(x) == "Siid")
    {
      if(is.null(x$varnames)) 
      {
        varnames <- paste("Density of" ,as.character(1:dim(x$data)[2]))
      }else{
        varnames <- x$varnames
      }
      data <- x$data
...
plot_pmf <- function(q, p, ylab, main= NA, which= NULL) {
  ...
  mq = q[which(p == mp)]
  mp = rep(mp,length(mq))
  points(mq, mp, pch = 16, cex = 1.2, col = "steelblue")
}