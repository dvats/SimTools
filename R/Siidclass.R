...
#' @param which : if one want to plots specific components
#' @param main : for main heading
...
#'
#' @export
"plot.Siid" <- function(x, which = NULL, Q = c(0.1, 0.9), alpha = 0.05, thresh = 0.001, rug = TRUE, 
  plot = TRUE,  mean = TRUE, border = NA, mean.col = 'plum4', quan.col = 'lightsteelblue3',
 opaq = 0.7, auto.layout = TRUE, ask = dev.interactive(),main = NA, ...)
{
  x <- as.Siid(x)
  out <- getCI(x, Q, alpha, thresh = thresh, iid = TRUE, mean = mean)
  if(plot == TRUE)
  {
    plot.CIs(x, which = which, CIs = out, rug = rug, bord = border, 
