...
"plot.Smcmc" <- function(x,...)
{
  ...
  p2 <- 0
  p3 <- 0
  if(p>10){stop("Maximum allowed dimension is 10")}
  if(p>4){p2 = p-4;p = 4}
  if(p2 >4)
  {
    p3 <- p2-4
  ...
  par(mfrow = c(3,p),mar = c(2,4,1,1))
  traceplot(x,which = 1:p,legend = F,xlab = NA,...)
  acfplot(x,which = 1:p,xlab = NA,...)
  densityplot(x,which = 1:p,...)
  
  if(p2>0)
  {
    par(mfrow = c(3,p2),mar = c(2,4,1,2))
    traceplot(x,which = 5:(4+p2),legend = F,xlab = NA)
    acfplot(x,which = 5:(4+p2),xlab = NA)
    densityplot(x,which = 5:(4+p2))
  }
  
  if(p3>0)
  {
    par(mfrow = c(3,p3),mar = c(2,4,1,2))
    traceplot(x,which = 9:(8+p3),legend = F,xlab = NA)
    acfplot(x,which = 9:(8+p3),xlab = NA)
    densityplot(x,which = 9:(8+p3))
  }
