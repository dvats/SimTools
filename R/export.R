...
{
  varname <- NULL
  if(TRUE %in% (class(x) == "Smcmc")){if(!is.null(x$varnames)){varname = x$varnames};x <- x$chains}else if(is.list(x))
  {
  ...
  for(i in dimen)
  {
    ...
    { 
      ...
      plot(avgf, ci = 0, lwd = .2, ylim = c( min(- 1.96/sqrt(n),min(avgf$acf),k), max(max(avgf$acf),l)), ylab = varname[i],
           lty = 1, xaxt = if(i>axs) 's' else 'n',main = t, xlab = NA)
      ...
      if(is.null(which)){if(np%%2 != 0 && i == np-1 && np>4){mtext(xlab, side = 1, line = 2.3,cex = 0.8)}}
      else if(!is.null(which)){mtext(xlab, side = 1, line = 2.3,cex = 0.8)}
    }
    ...
    if(plot){
      ...
      if(is.null(which))
      {
        if(np <= 4){mtext(xlab, side = 1, line = 2.3,at =0.52, outer = TRUE, cex= 1)}
        else{mtext(xlab, side = 1, line = 2.3,at = 0.3,outer = TRUE,cex = 0.8)
          if(np %% 2 == 0){mtext(xlab, side = 1, line = 2.3,at = 0.8,outer = TRUE,cex = 0.8)}}
      }
    }
  }
                      ...
                      col = c("palevioletred3","steelblue3","tan3","seagreen3","tomato3"), ...)
{
  ...
  if(TRUE %in% (class(x) == "Smcmc")){if(!is.null(x$varnames)){ylab = x$varnames};x <- x$chains}else if(is.list(x))
  {
    for(i in 1:length(x)) {x[[i]] = as.matrix(x[[i]])}
  }else if(is.vector(x))
  {
  ...
  if(!is.null(which)| p > 10)
  {
    ...
    {
      if(is.null(main) && legend){par(oma = c(0,0,2,0));space = 1}
      else if(legend && !is.null(main)){par(oma = c(0,0,4,0));space = 1.5;l=-1.6}
      else if(!legend && !is.null(main)){par(oma = c(0,0,1,0));l = 0} 
    }
    if(leg == 1){par(mar = c(4,4,2,2))}
    for(i in which)
    {
      j <- 1  
      ...
      if(leg == 1)
      {
        if(!is.null(main)){mtext(main, side = 3, line = l,outer = TRUE, cex = 1.5)} 
        ##Setting of legend
        if(legend)
        {
  ...
  else
  {
    ...
    else if(legend && !is.null(main)){par(oma = c(3.5,0,4.2,0))
      space = 1}
